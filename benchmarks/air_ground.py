...
class AirGroundPoseMNNBenchmark:
    ...
    def benchmark(self, model_helper, model_name = None, scale_intrinsics = False, calibrated = True, plot_every_iter=10, plot=False, method='sparse', pose_estimator='opencv'):
        with torch.no_grad():
                ...
                for pair in tqdm.tqdm(indices):
                    ...
                    if ret is not None:
                        ...
                        if scene_ind % plot_every_iter == 0 and plot:
                            ...
                            if not os.path.exists(f'outputs/air_ground/{model_name}_{method}_{pose_estimator}'):
                                os.makedirs(f'outputs/air_ground/{model_name}_{method}_{pose_estimator}')
                            name = f'outputs/air_ground/{model_name}_{method}_{pose_estimator}/{scene_name}_{idx}.png'
...
if __name__ == "__main__":
    ...
    if not os.path.exists(f'outputs/air_ground'):
        os.makedirs(f'outputs/air_ground')
    ...
    with torch.no_grad():
        ...
        with open(f'outputs/air_ground/RDD_{method}_{args.pose_estimator}.txt', 'w') as f:
            f.write(str(out))
