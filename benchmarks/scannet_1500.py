        ...
        with torch.no_grad():
            ...
            for pairind in tqdm(range(len(self.scenes))):
                ...
                if method == 'dense':
                    ...
                    kpts0, kpts1, conf = model_helper.match(im_A, im_B, thr=0.01, resize=None)
                else:
                ...
                if ret is not None:
                    ...
                    if pairind % plot_every_iter == 0 and plot:
                        ...
                        if not os.path.exists(f'outputs/scannet/{model_name}_{method}'):
                            os.makedirs(f'outputs/scannet/{model_name}_{method}')
...
def parse_arguments():
    ...
    parser.add_argument("--weights", type=str, default="./weights/RDD-v3.pth", help="Path to the model checkpoint.")
    ...
    parser.add_argument("--model", type=str, default="RDD", help="Model to use: RDD or RDD_DINO.")
...
if __name__ == "__main__":
    ...
    if args.model == "RDD_DINO":
        model = build_rdd_dino(weights=args.weights)
    else:
