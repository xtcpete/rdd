...
class RDDLightningModule(pl.LightningModule):
    ...
    def __init__(
        ...
        self.descriptor_loss = DescriptorLoss(inv_temp=20) if stage == "descriptor" else None
        self.warper = warper if stage == "descriptor" else None
    ...
    def configure_optimizers(self):
        ...
        scheduler = MultiStepLR(optimizer, milestones=[3, 6, 12], gamma=self.hparams.gamma_steplr)
        return {
            "optimizer": optimizer,
            "lr_scheduler": {
                "scheduler": scheduler,
    ...
    def _descriptor_training_step(self, batch: Dict[str, torch.Tensor], batch_idx: int):
        ...
        for idx, positives in enumerate(positives_md_coarse):
            ...
            m1 = feats1[idx, :, pts1[:, 1], pts1[:, 0]].permute(1, 0)
            m2 = feats2[idx, :, pts2[:, 1], pts2[:, 0]].permute(1, 0)
